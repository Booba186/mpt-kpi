<template>
	<div class="">
		...
		<span class="warning"> Если ты видешь это в проде, то срочно пиши @vasylyev </span>
...
export default {
	...
	mounted() {
		this.fieldsList = JSON.parse(localStorage.getItem('customRequestFieldsList')) ?? []
	},
	...
	methods: {
		saveFieldsListToLocalStorage() {
			localStorage.setItem('customRequestFieldsList', JSON.stringify(this.fieldsList))
		},

		addField(fieldData) {
			const fieldIndex = this.fieldsList.findIndex((field) => field.name === fieldData.name)
		...
		deleteField(fieldName) {
			const fieldIndex = this.fieldsList.findIndex((field) => field.name === fieldName)
